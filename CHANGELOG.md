...
- `/calendar`를 월간 캘린더 중심의 한 열 화면으로 재배치하고, 날짜 선택 시 세부 일정과 공식 출처를 확인할 수 있게 했습니다. Calendar API 이벤트와 주요 반복 일정에는 `source_name`, `source_url` 필드를 추가했습니다.
- `/archive`에서 최신 기관 리포트 상단 카드, 예측 검증, 내부 분석 리포트 영역을 제거하고, 지역 필터와 공식 리서치 원문 목록 중심의 리서치 아카이브로 단순화했습니다.

## v2.68.3 - 2026-06-03

- 랜딩 히어로의 큰 제목이 PC 폭에서 과도하게 커지거나 preview 칼럼을 압박하지 않도록 타이포그래피와 첫 화면 높이, 미리보기 카드 내부 줄바꿈을 정리했습니다.
- 대시보드의 주요 섹션을 데스크톱에서도 한 열 흐름으로 재배치하고, 포커스·뉴스·상위 종목·섹터 로테이션 목록은 내부 반응형 그리드로 펼쳐 넓은 화면의 빈 공간을 줄였습니다.
- 섹터 로테이션 행과 모바일 막대/퍼센트 배치를 별도 CSS로 분리해 긴 섹터명, 종목명, 등락률이 카드 밖으로 밀리지 않게 보강했습니다.
- 랜딩, 대시보드, 내비게이션의 소개형 문구를 제품명·화면명·데이터 단위 중심의 운영형 카피로 정리했습니다.
...
- 대시보드 전용 CSS 네임스페이스를 추가해 기존 앱 라우트 스타일을 흔들지 않고 히어로, 시장 신호, 포커스, 히트맵, 상승·하락 상위, 강한 셋업 패널을 새 디자인 기준에 맞췄습니다.
- 섹터 로테이션 보드를 새 대시보드 패널 표면으로 정리해 통계 타일과 섹터 막대가 같은 시각 언어로 보이도록 맞췄습니다.
...
- `/stock/[ticker]` shell 자동 재조회에 10초 중간 지점을 추가해 `5초 -> 8초 -> 10초 -> 18초 -> 32초` 단계로 동작합니다. 빠른 종목은 10초대에, 느린 종목은 30초대 초반에 숫자 승격을 잡도록 조정했습니다.
