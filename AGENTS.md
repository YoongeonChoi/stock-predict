...
- `/`
  - public landing
  - 서비스 소개, 핵심 기능, 사용 흐름, 신뢰/제한, Contact 진입
- `/dashboard`
  - 대시보드
...
- 표시 confidence는 고정 heuristic를 그대로 노출하지 않고, raw support를 만든 뒤 실측 prediction log 기준의 empirical sigmoid로 다시 맞춥니다.
- 표본 수와 클래스 균형이 충분하면 horizon별 isotonic 단계와 reliability bin 메타데이터까지 함께 갱신해, 연구실과 시스템 진단에서 calibration gap을 같이 확인할 수 있게 유지합니다.
...
이 저장소의 릴리즈 버전은 `MAJOR.MINOR.PATCH` 형식을 사용합니다.

- `PATCH`
  - 버그 수정
  - timeout / fallback / 운영 안정화
  - 문서, 테스트, 진단 보강
  - 기존 흐름을 바꾸지 않는 UI 문구/배치 정리
- `MINOR`
  - 새 사용자 기능 추가
  - 새 페이지 / 새 API / 새 응답 필드 추가
  - 예측 엔진 / 포트폴리오 엔진의 의미 있는 개선
  - 회원가입 / 계정 / 내보내기 같은 핵심 작업 흐름 확장
- `MAJOR`
  - 기존 API 계약을 깨는 변경
  - 라우트, 메뉴, 저장 구조를 비호환 방식으로 바꾸는 변경
  - 운영자가 별도 마이그레이션이나 전환 안내를 해야 하는 변경

추가 규칙:

- 런타임 동작이 바뀌지 않는 내부 메모, 오탈자 수준 문서 수정만 단독으로 할 때는 버전을 올리지 않아도 됩니다.
- 사용자 동작, 운영 안내, 설정 화면의 버전 표시처럼 실제 배포 인식에 영향을 주는 변경은 현재 릴리즈 버전에 함께 반영합니다.
- 버전을 올릴 때는 아래 파일을 한 묶음으로 동기화합니다.
...
10. 프론트엔드나 UI/UX를 수정할 때는 반드시 `DESIGN_BIBLE_NEXT.md`를 먼저 읽고, v2.68.0 전면 리디자인 기준을 따릅니다. `DESIGN_BIBLE.md`는 삭제하지 않지만 legacy reference로만 봅니다.
11. 새로운 UI primitive나 시각 규칙을 추가했다면 `DESIGN_BIBLE_NEXT.md` 또는 관련 공용 파일에 기준을 함께 반영합니다.
12. 공개 대시보드나 집계형 API를 추가/수정할 때는 반드시 timeout과 부분 fallback을 함께 설계합니다.
   ...
   - Render free 환경에서 공개 경로는 가능한 한 `대표 표본 + 캐시 + 구조화된 timeout`을 우선합니다.
   - 다만 `기회 레이더`는 예외적으로 `KR 유니버스 전체 1차 quote screen + 상위 종목 정밀 분석` 2단계 구조를 유지합니다.
...
- 로그인 후 `/settings`에서 아이디, 이름, 전화번호, 생년월일 수정, 이메일 변경 요청, 인증 메일 재전송, 새 비밀번호 즉시 변경, 필요 시 보안 확인 코드(reauth nonce) 요청, 비밀번호 재설정 메일 발송, 현재 세션 확인, 이 기기 로그아웃, 모든 기기 로그아웃, 회원 탈퇴를 처리합니다.
- `/settings`에서 저장되지 않은 입력이 남아 있으면 상단 경고 배너와 브라우저 이탈 보호를 보여 주고, 프로필/이메일/비밀번호/탈퇴 입력은 섹션별 초기화 버튼으로 바로 비울 수 있어야 합니다.
...
- `/settings`에서 비밀번호를 즉시 변경할 때는 오래된 세션에서 Supabase reauth nonce가 필요할 수 있으므로, 같은 패널 안에서 `보안 확인 코드 보내기 -> nonce 입력 -> 저장` 흐름을 유지합니다.
- `/settings`에서 draft state를 다룰 때는 사용자가 저장하지 않은 입력을 잃지 않도록 `경고 배너`, `beforeunload 보호`, `섹션별 reset` 중 최소한 하나가 아니라 세 가지를 함께 고려합니다.
- 보호된 API가 `401 / SP-6014`를 반환하면 프론트는 페이지별 ad-hoc 처리보다 `AuthProvider`에서 세션 재검증/복구를 우선 시도하고, 복구되지 않으면 로그인 유도 상태로 자연스럽게 내려가야 합니다.
...
2. UI 작업이면 `DESIGN_BIBLE_NEXT.md`를 먼저 확인합니다.
3. 기능 코드를 수정합니다.
...
- [ ] UI 변경이 `DESIGN_BIBLE_NEXT.md`와 일관된가
...
운영 배포 상태를 직접 점검하려면 아래 옵션을 사용합니다.
