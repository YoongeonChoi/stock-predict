...
- 투자 성향 저장/조회 실패는 `500 / SP-5021`
- 개인화 포트폴리오 추천 실패는 `500 / SP-5022`
- 투자 성향 입력 검증 실패는 `400 / SP-6020`
...
- `GET /api/stock/{ticker}/detail`
  - `weekly_trade_plan`
    ...
    - v2.66.5부터 quick warm은 공개 응답 timeout과 분리된 백그라운드 예산을 사용합니다. 숫자 없는 shell cache를 다시 서빙할 때도 최상위 `fallback_reason=stock_memory_guard`를 유지해, 프론트가 일반 quick partial이 아니라 자동 재조회 대상 shell로 계속 처리할 수 있습니다.
    - v2.66.6부터 quick 분포 응답도 이미 캐시에 있는 공식/허용 리서치 메타데이터를 짧은 timeout으로 조회해 `official_research` evidence와 `source_freshness`에 반영할 수 있습니다. 이 quick 조회는 `auto_refresh=false`라 외부 리서치 동기화를 응답 경로에 붙이지 않습니다.
    - v2.66.7부터 Render safe mode의 cold import/ultra-fast fallback 분기도 shell을 만들기 전에 기존 quick cache를 먼저 확인합니다. 캐시된 quick 판단이 있으면 `prefer_full=true`가 full 분석을 건너뛰는 경우에도 최상위 `fallback_reason`은 실제 quick 상태를 유지하고, shell일 때만 `stock_memory_guard`를 사용합니다.
    - v2.66.8부터 safe-mode quick warm은 백그라운드에서 14초까지 허용하고, 프론트 shell 자동 재조회는 `6초 -> 14초 -> 28초 -> 45초` 단계로 동작합니다. 응답 경로를 늘리지 않으면서 첫 숫자 캐시 승격을 더 빨리 화면에 반영하기 위한 계약입니다.
    - v2.66.9부터 프론트 shell 자동 재조회는 `5초 -> 8초 -> 16초 -> 32초` 단계로 조정됩니다. v2.66.8의 14초 백그라운드 warm 예산은 유지하면서, 두 번째 재조회가 더 일찍 도달해 숫자 있는 quick 분포 캐시를 더 빨리 잡도록 합니다.
    - v2.66.10부터 safe-mode quick warm은 백그라운드에서 28초까지 허용하고, 프론트 shell 자동 재조회는 `5초 -> 8초 -> 10초 -> 18초 -> 32초` 단계로 동작합니다. 응답 경로는 계속 shell/partial 우선이며, 느린 종목도 첫 warm 안에서 숫자 캐시 승격을 잡는 쪽을 우선합니다.
    - v2.66.11부터 Render safe-mode startup의 public dashboard prewarm은 대표 종목 quick detail warm도 함께 예약합니다. 현재 대상은 `003670.KS`, `005930.KS`이며, 메모리 압박 guard가 허용할 때만 백그라운드에서 숫자 있는 quick 분포 캐시 준비를 시작합니다.
...
- `GET /api/portfolio/recommendations/personalized`
- `GET /api/investment-profile`
- `PUT /api/investment-profile`
- `GET /api/investment-profile/options`
- `POST /api/investment-profile/resolve`

`GET /api/investment-profile` response:
...
`PUT /api/investment-profile` request:
...
`profile_code`는 `capital_preservation`, `conservative`, `balanced`, `growth`, `aggressive` 중 하나입니다.
요청에 세부 필드를 생략하면 서버가 해당 성향 preset 기본값을 채웁니다.

투자 성향은 예측 backbone을 바꾸지 않고 추천 정책에만 반영합니다.

- 후보 통과 기준: `min_confidence`, `min_up_probability`, `max_down_probability`
- 비중 제약: `recommended_equity_pct`, `cash_buffer_pct`, `max_single_weight_pct`, `max_country_weight_pct`, `max_sector_weight_pct`
- optimizer 파라미터: `risk_aversion`, `turnover_penalty`, `expected_excess_weight`, `expected_total_weight`, `probability_edge_weight`
- 응답 설명: `recommendation_policy`, 종목별 optional `profile_fit`

`GET /api/portfolio/recommendations/conditional`, `optimal`, `personalized`는 기존 필드를 유지하고 optional `recommendation_policy`를 추가할 수 있습니다.
`recommendation_policy.dynamic_adjustments[]`는 risk-off 시장, 높은 포트폴리오 위험, 낮은 포트폴리오 상승 확률 같은 동적 보정 사유를 설명합니다.
추천 snapshot 저장은 best-effort이며, snapshot 저장 실패가 추천 응답 실패로 이어지면 안 됩니다.
...
- `GET /api/calendar/{code}`
  - `events[]`, `upcoming_events[]`, `economic_events[]`는 일정 상세 표시를 위해 `source_name`, `source_url`을 optional로 포함할 수 있습니다.
  - `major_events[]`도 반복 주요 일정의 공식 출처 연결을 위해 `source_name`, `source_url`을 optional로 포함할 수 있습니다.
- `GET /api/archive`
