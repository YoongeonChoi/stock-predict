...
- `GET /api/stock/{ticker}/detail`
  ...
  - `fallback_reason=stock_quick_detail`
    ...
    - route는 quick snapshot 반환 뒤 full analyze + archive save를 background refresh로 이어가며, stock page는 이 partial을 server-first initial snapshot으로 first HTML에 먼저 실을 수 있어야 합니다.
