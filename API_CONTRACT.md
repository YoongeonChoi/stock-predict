...
`GET /api/diagnostics`는 기존 시스템 상태 필드 외에 `route_stability` 배열을 포함할 수 있습니다.

- 각 항목은 `route`, `total_requests`, `success_count`, `error_count`, `degraded_count`, `fallback_served_count`, `stale_served_count`, `cold_start_count`, `cold_failure_count`, `first_usable_p50_ms`, `first_usable_p95_ms`를 가질 수 있습니다.
- 비율 필드는 `degraded_rate`, `fallback_served_rate`, `stale_served_rate`, `cold_failure_rate`를 사용합니다.
- 세부 분류는 `phase_counts`, `cache_counts`, `last_fallback_reason`, `last_upstream_source`, `last_served_state`, `last_elapsed_ms`, `last_updated_at`로 확인합니다.
- 프론트 `/settings`는 이 필드를 사용해 `핵심 Route 안정성` 패널을 구성하며, 운영 중 `blank screen`, `error-only`, `hydration 후 timeout` 징후를 빠르게 진단하는 데 사용합니다.
=======
