...
export default function ArchiveExportPage() {
  ...
  useEffect(() => {
    ...
    if (!reportId || autoTriggered.current || (format !== "pdf" && format !== "csv")) return;

    autoTriggered.current = true;
    downloadApiFile(`/api/export/${format}/${reportId}`, `report_${reportId}.${format}`)
      .then(() => toast(`${format.toUpperCase()} 다운로드를 시작했습니다.`, "success"))
      .catch((downloadError) => {
        console.error(downloadError);
        toast("자동 다운로드에 실패했습니다. 아래 버튼을 눌러 다시 시도해 주세요.", "error");
      });
  }, [reportId, searchParams, toast]);
