...
const KOREAN_DATE_TIME_PARTS_FORMATTER = new Intl.DateTimeFormat("en-CA", {
...
const KOREAN_DATE_PARTS_FORMATTER = new Intl.DateTimeFormat("en-CA", {
  timeZone: KOREA_TIME_ZONE,
  year: "numeric",
  month: "2-digit",
  day: "2-digit",
...
const KOREAN_TIME_PARTS_FORMATTER = new Intl.DateTimeFormat("en-CA", {
  timeZone: KOREA_TIME_ZONE,
  hour: "2-digit",
  minute: "2-digit",
  second: "2-digit",
...
function extractDateParts(formatter: Intl.DateTimeFormat, date: Date) {
...
const PARTIAL_REASON_CHIPS: Record<string, PublicAuditChip> = {
  ...
  daily_briefing_timeout: { label: "브리핑 임시 스냅샷", tone: "info" },
  daily_briefing_error: { label: "브리핑 임시 스냅샷", tone: "info" },
  country_report_startup_seed: { label: "준비 스냅샷", tone: "info" },
  country_report_startup_guard: { label: "초기 스냅샷", tone: "info" },
  country_report_stale_public: { label: "직전 리포트", tone: "info" },
  ...
  kr_last_success_snapshot: { label: "직전 스냅샷", tone: "info" },
  kr_representative_snapshot_warming: { label: "대표 스냅샷", tone: "info" },
  kr_bulk_snapshot_only: { label: "대표 스냅샷", tone: "info" },
  kr_bulk_snapshot_warming: { label: "대표 스냅샷", tone: "info" },
  kr_timeout_shell: { label: "빠른 스냅샷", tone: "info" },
  screener_timeout_shell: { label: "빠른 스냅샷", tone: "info" },
  snapshot_only: { label: "기본 스냅샷", tone: "info" },
  stock_cached_detail: { label: "저장 스냅샷", tone: "info" },
  stock_memory_guard: { label: "보호 스냅샷", tone: "neutral" },
...
const FALLBACK_REASON_LABELS: Record<string, string> = {
  ...
  daily_briefing_timeout: "브리핑 임시 스냅샷 기준",
  daily_briefing_error: "브리핑 임시 스냅샷 기준",
  ...
  country_report_stale_public: "직전 공개 리포트 기준",
  ...
  kr_last_success_snapshot: "직전 스크리너 스냅샷 기준",
  kr_bulk_snapshot_only: "대표 종목 스냅샷 기준",
  kr_bulk_snapshot_warming: "대표 종목 스냅샷 기준",
  kr_timeout_shell: "빠른 스크리너 스냅샷 기준",
  screener_timeout_shell: "빠른 스크리너 스냅샷 기준",
  snapshot_only: "기본 스크리너 스냅샷 기준",
  stock_cached_detail: "최근 저장 상세 스냅샷 기준",
  stock_memory_guard: "기본 종목 스냅샷 기준",
...
function normalizeReason(reason?: string | null) {
  ...
  return FALLBACK_REASON_LABELS[normalized] || null;
}
...
export function formatAuditTimestamp(value?: string | null) {
  ...
  const { year, month, day, hour, minute, second } = extractDateParts(KOREAN_DATE_TIME_PARTS_FORMATTER, date);
...
export function formatAuditDate(value?: string | null) {
  ...
  const { year, month, day } = extractDateParts(KOREAN_DATE_PARTS_FORMATTER, date);
...
export function formatAuditTime(value?: string | null) {
  ...
  const { hour, minute, second } = extractDateParts(KOREAN_TIME_PARTS_FORMATTER, date);
...
export function buildPublicAuditSummary(
  ...
  if (
    meta?.partial &&
    (meta?.fallback_reason === "kr_timeout_shell" ||
