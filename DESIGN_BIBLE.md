...
- 기회 레이더 품질 신호처럼 비교용 보조 metric은 카드 안에 또 카드를 만들지 않고 낮은 대비의 한 줄 strip으로 배치한다. `추격 위험`, `거래량`, `수급`, `섹터`처럼 짧은 명사 label과 2~4자 상태값 또는 0~100 점수만 둔다.
- KRX 수급처럼 지연이 있는 metric은 raw key 대신 `18시 이후 갱신`, `EOD 반영`, `수급 대기`처럼 사용자가 이해하는 상태명으로 표시한다.
...
- 후보의 5거래일 매수·매도 요약은 optional 보조 strip으로만 둔다. 필드가 없을 때 카드 높이나 기존 판단 흐름이 흔들리면 안 된다
...
- thread는 `종목 헤더 -> 이번 주 판단 -> 판단 요약 -> 차트 -> 확률 엔진 -> 상세 지표` 순서로 둔다
- 감사/제한 배너가 있더라도 첫 판단 카드보다 위에 두지 않는다. 사용자는 종목 진입 직후 매수 가능가, 매도 목표가, 손절가를 먼저 확인해야 한다
- `이번 주 판단`은 한 slab 안에서 대표 매수가, 매도가, 손절가, 확률, 신뢰도, 데이터 상태를 끝낸다
- 5거래일 가격 제안은 수익 보장처럼 말하지 않고 `조건부 분포 기반 실행 가격 제안`으로 표현한다
- 공식 리서치나 IB 메타데이터는 숫자보다 근거 상태로 보여 준다. 사용자는 `반영/부분/대기/미설정` 상태를 먼저 보고, 원문 PDF 본문을 무단 수집한 듯한 표현은 쓰지 않는다. 이 소스는 배열 뒤쪽에 있어도 카드에서 생략하지 않는다
- `예측 변화 추적`의 5거래일 실행안 검증은 `목표 도달`, `손절 접촉`, `실제 범위`처럼 관찰된 사실로만 쓴다. 수익 보장, 정답률 과장, 일중 선후관계 단정 표현은 쓰지 않는다
