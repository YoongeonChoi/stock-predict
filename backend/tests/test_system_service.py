...
class SystemServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_diagnostics_skips_heavy_probes_under_elevated_warning_pressure(self):
        route_summary = {
            "routes": [],
            "first_usable_metrics": {
                "tracked_routes": 0,
        ...
        elevated_memory_snapshot = {
            "source": "proc_status",
            "rss_bytes": 410 * 1024 * 1024,
            "rss_mb": 410.0,
            "peak_rss_bytes": 450 * 1024 * 1024,
            "peak_rss_mb": 450.0,
            "cgroup_current_bytes": 420 * 1024 * 1024,
            "cgroup_current_mb": 420.0,
            "cgroup_limit_bytes": 500 * 1024 * 1024,
            "cgroup_limit_mb": 500.0,
            "configured_budget_mb": 500,
            "resolved_budget_mb": 500.0,
        ...
        with (
            ...
            patch("app.services.system_service.route_stability_service.get_route_stability_summary", return_value=route_summary),
            patch("app.services.system_service.maybe_trim_process_memory", return_value={"attempted": True, "trimmed": True}) as trim_memory,
            patch("app.services.system_service._get_process_memory_snapshot", side_effect=[elevated_memory_snapshot, elevated_memory_snapshot, elevated_memory_snapshot]),
        ):
            diagnostics = await system_service.get_diagnostics()

        self.assertEqual(diagnostics["prediction_accuracy_error"], "skipped under warning memory pressure")
        self.assertEqual(diagnostics["research_archive_error"], "skipped under warning memory pressure")
        trim_memory.assert_called_once_with("diagnostics_preflight")
