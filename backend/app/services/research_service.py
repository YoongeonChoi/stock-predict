...
def _build_insights(accuracy: dict, by_country: list[dict], calibration: list[dict], recent_records: list[dict]) -> list[str]:
    insights: list[str] = []
...
async def get_prediction_lab(limit_recent: int = 40, refresh: bool = True) -> dict:
    cache_key = f"prediction_lab:v1:{limit_recent}:{int(refresh)}"
    ...
    accuracy, recent_rows, trend_rows, by_country_rows, by_scope_rows, by_model_rows, calibration_rows = await asyncio.gather(
    ...
    response = {
        ...
        "insights": _build_insights(accuracy, by_country, calibration, recent_records),
    }
