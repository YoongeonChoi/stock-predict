...
- Render memory-safe 운영에서 `/api/country/KR/report`는 이제 cached/archived 리포트가 아직 비어 있어도, background refresh를 계속 살릴 수 있는 구간이면 공개 대기 시간을 더 짧은 safe-mode budget으로 먼저 자릅니다. 그래서 cold/full 분석이 5초 budget을 매번 다 쓰며 `country_report_timeout` partial로 떨어지던 구간을 줄이고, 첫 응답은 더 빨리 partial로 닫되 다음 조회에서 full cache가 회복될 가능성은 유지하도록 맞췄습니다.
- `backend/tests/test_country_router.py`에는 일반 공개 budget과 safe-mode 단축 budget이 각각 언제 적용되는지 확인하는 회귀를 추가했습니다. 앞으로는 Render 500MB 운영 한도에서 country report가 다시 긴 대기 시간으로 되돌아가거나, 반대로 일반 환경까지 과하게 짧아지는 회귀를 테스트에서 바로 잡을 수 있습니다.
