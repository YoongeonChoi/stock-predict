...
공유 링크 대표 이미지: `frontend/public/og/sp-og.png`
...
| 화면 | 역할 | 비고 |
|---|---|---|
| `/` | public landing, 핵심 기능, 사용 흐름, 신뢰/제한, Contact 진입 | 공개 |
| `/dashboard` | 새 디자인 언어의 대시보드, 브리핑, 시장 스냅샷, 히트맵, 강한 셋업 | 공개, 모바일 강한 셋업은 핵심 요약 우선 |
| `/radar` | Opportunity Radar | 공개, 대표 유니버스 기반, 모바일 후보 목록은 티커·이름·가격·최종 점수 중심 |
...
- `/`는 첫 방문자가 서비스 목적과 주요 기능을 빠르게 이해하도록 만든 public landing입니다.
- Hero, 핵심 기능, 사용 흐름, 제품 미리보기, 신뢰/제한, 최종 CTA를 통해 실제 작업 화면으로 연결합니다.
- 전역 Contact footer로 `contact@yoongeon.xyz` 메일 링크와 문의 모달을 제공합니다.
...
- `/calendar`는 정책 발표, 경제지표, 주요 실적 일정을 월간 캘린더로 표시합니다.
- 날짜를 선택하면 해당 날짜의 세부 일정, 중요도, 국가/지역, 공식 출처를 같은 화면에서 확인할 수 있습니다.
- 주요 반복 일정은 한국은행, 통계청, Federal Reserve, ECB, BOJ 등 공식 출처 링크를 함께 제공합니다.

### 9. 리서치 아카이브

- `/archive`는 공식 기관 리포트와 정책·시장 분석 원문을 지역별로 정리합니다.
- 지역 필터, 발행일, 출처, PDF 여부, 원문 링크를 중심으로 보여 주며 최신 리포트 홍보 카드나 내부 분석 리포트 다운로드 영역은 별도 노출하지 않습니다.
- 서버 렌더 단계에서는 리서치 아카이브와 소스 상태만 timebox로 가져와 공개 화면의 목적을 단순하게 유지합니다.
...
- 모든 화면 하단에서 `Yoongeon Choi`, `contact@yoongeon.xyz`, GitHub, LinkedIn 링크를 footer 형식으로 제공합니다.
- `문의 보내기` 버튼을 누르면 이름, 이메일, 제목, 메시지를 입력하는 모달을 열어 문의를 접수합니다.
...
- `/dashboard`, `/radar`, `/stock/[ticker]`처럼 첫 화면 자체가 제품 경험의 핵심입니다.
...
v2.68.0부터 `/`는 public landing으로 전면 교체하고, 기존 대시보드는 `/dashboard`로 이동했습니다.
기존 [DESIGN_BIBLE.md](./DESIGN_BIBLE.md)는 삭제하지 않고 legacy reference로 보존하지만, 새 UI/UX 작업의 active 기준은 [DESIGN_BIBLE_NEXT.md](./DESIGN_BIBLE_NEXT.md)입니다.
Figma handoff 파일은 [yoongeon.xyz Full Redesign v2.68](https://www.figma.com/design/E51YcIXya4tLruomocZgrb)입니다.

핵심 시각 원칙:

- true white 바탕
- cobalt accent 1개
- 섹션당 headline 1개, supporting copy 1개, primary CTA 1개
- 모바일 360px 기준 정보 재배치
- 버튼/입력 최소 48px touch target
...
| GitHub source | Figma 산출물은 디자인 source, GitHub repo는 production source |

더 자세한 UI 기준은 [DESIGN_BIBLE_NEXT.md](./DESIGN_BIBLE_NEXT.md)를 참고하세요.
...
- [DESIGN_BIBLE_NEXT.md](./DESIGN_BIBLE_NEXT.md)
