...
- 종목 검색 / 비교
- 이메일 회원가입 / 로그인
- 계정 프로필 수정 / 이메일 인증 재전송 / 비밀번호 변경 / 비밀번호 재설정 / 세션 종료 / 회원 탈퇴
- 사용자별 관심종목
- 사용자별 포트폴리오 / 자산 프로필
- 포트폴리오 조건 추천 / 최적 추천
- 일일 이상적 포트폴리오
...
- 백엔드는 인증 토큰을 검증한 뒤 `/api/account/me`에서 마스킹된 전화번호와 이메일 인증 상태를 함께 반환
...
- `GET /api/account/me`
  - 현재 로그인 계정 프로필 반환
  - `email_verified`, `email_confirmed_at`, 마스킹된 전화번호 포함
