...
이 버전부터 앱은 `KR`만 지원하며, 로그인한 사용자별로 워치리스트와 포트폴리오를 분리해 저장합니다. 프론트는 `Vercel`, 백엔드는 `Render`, 사용자 데이터와 인증은 `Supabase`, 도메인은 `Cloudflare` 기준으로 운영 흐름을 정리했습니다.
...
- `OPENAI_API_KEY`가 없어도 일부 정량 기능은 동작하지만 서술형 설명 품질은 낮아집니다.
