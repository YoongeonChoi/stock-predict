...
- 포트폴리오 추천, 조건 추천, 최적 추천, 일일 이상적 포트폴리오는 `20거래일 기대수익률 + 기대초과수익률 + EWMA+shrinkage 공분산 + 회전율 패널티` 기준의 공통 optimizer를 사용합니다.
- 포트폴리오 비중 계산의 canonical 구현은 `backend/app/services/portfolio_optimizer.py` 입니다.
...
- 포트폴리오 추천 계열 화면은 20거래일 기대수익률, 기대초과수익률, 상방·하방 확률, 예상 변동성, 회전율을 같은 기준으로 읽도록 맞춰져 있습니다.
...
- Render free 환경에서 첫 호출이 느릴 수 있어도, 포트폴리오 수치 자체는 서버 내부 분포 엔진과 optimizer가 계산하며 LLM이 비중을 직접 정하지는 않습니다.
