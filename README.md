...
- Render memory-safe quick 응답도 6개월 가격·시장 시계열로 5거래일 분포를 먼저 계산합니다. 그래서 정밀 뉴스·공시·리서치 보강이 아직 끝나지 않아도 첫 판단 카드에는 q25/q50/q75/q90 기반 매수 가능가, 매도 목표가, 손절가, 상승/보합/하락 확률이 비어 있지 않게 내려옵니다.
- cold start 직후 stock analyzer가 아직 import되지 않아 최소 shell을 먼저 반환하더라도, 메모리 압박이 높지 않으면 응답 뒤 quick 분포 warm을 예약합니다. 다음 조회나 hydration upgrade는 같은 종목의 quick 분포 캐시를 더 빨리 받을 수 있습니다.
- 이미 최소 shell이 quick cache에 남아 있는 종목도 다음 조회에서 다시 quick 분포 warm을 예약하고, shell cache TTL을 30초로 줄여 숫자 없는 대기 카드가 오래 유지되지 않게 했습니다.
- `/stock/[ticker]` 클라이언트는 최소 shell을 받으면 최초 full 업그레이드 뒤에도 짧은 지연 재조회를 최대 2회 예약합니다. 백엔드 warm cache가 숫자 있는 5거래일 판단으로 승격되면 사용자가 새로고침하지 않아도 같은 화면에서 매수 가능가, 매도 목표가, 손절가가 채워집니다.
- `/radar`가 startup placeholder를 받으면 화면이 멈춘 것처럼 보이지 않도록 짧은 자동 재조회를 예약합니다. 백엔드 quick warm-up이 후보 캐시를 채우면 사용자가 직접 새로고침하지 않아도 usable 후보 보드로 승격됩니다.
