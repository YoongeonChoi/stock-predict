...
- KR `screener` 기본 공개 경로는 Render 운영 안정성을 위해 SQLite shared cache를 거치지 않고 바로 계산합니다. 캐시 적중보다 응답 유지가 더 중요한 공개 quick path에 한해 이 원칙을 적용합니다.
...
- KR 기회 레이더는 KRX 전종목 1차 시세 확보 수가 `0`으로 떨어지면, 빈 후보를 그대로 두지 않고 즉시 운영용 기본 종목군으로 재시도합니다. 그래서 `실제 시세 확보 0 / 표시 후보 0` 상태가 오래 고정되지 않게 유지합니다.
