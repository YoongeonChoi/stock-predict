...
`--deployed-site-smoke`는 현재 운영 중인 `https://www.yoongeon.xyz`, `https://api.yoongeon.xyz`를 직접 호출해 프론트 HTML 응답, 핵심 공개 API, 인증 필요 API의 `401 / SP-6014` 계약을 함께 점검합니다. 이 스모크는 `KR heatmap`, `market opportunities`, `screener` 같은 느린 공개 API도 함께 확인하며, Render free 워밍업이나 배포 전환 구간의 일시적인 `502/503/504`와 timeout에는 짧게 재시도합니다. 최신 기준으로 `heatmap`과 `screener`는 느릴 때도 `partial` 응답으로 먼저 살아남는지 함께 봅니다.
