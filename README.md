...
│  │ /briefing/daily  /market/sessions  /ticker/resolve  │ │
│  │ /portfolio/event-radar  /stock/{ticker}/forecast-delta │ │
│  │ /portfolio/recommendations/conditional               │ │
│  │ /portfolio/recommendations/optimal                   │ │
│  └────────────────────────┬───────────────────────────┘ │
│                           ▼                             │
│  ┌─ Services ─────────────────────────────────────────┐ │
...
- 일봉 기반 차트와 다음 거래일 예측은 `마지막 완결 거래일` 기준으로 계산합니다.
- 장중에 Yahoo가 현재일의 미완결 일봉을 노출하더라도, 예측 입력에서는 제외합니다.
- 장 마감 뒤 당일 종가가 반영되면 가격 이력과 분석 캐시 키가 새 거래 세션 날짜로 자동 전환됩니다.
- 종목 상세의 가격 필드는 느린 펀더멘털 캐시가 아니라 신선한 시세 캐시를 우선 사용해, 전일 `currentPrice`가 당일 종가를 덮어쓰지 않도록 구성했습니다.
...
| **Stock Screener** | 국가, 섹터, 가격, 시총, P/E, P/B, 배당수익률, 베타, 52주 고점 대비, 성장률, ROE, 부채비율, 거래대금, 수익성까지 함께 거르는 실전형 조건 검색 |
| **Portfolio Tracker** | 총자산, 예수금, 월 추가 자금을 먼저 관리하고 보유 종목 CRUD, 손익, 비중, 실행 시그널, 섹터·국가 배분까지 한 화면에서 이어서 운영 |
...
| **조건 추천** | 국가, 섹터, 운영 성향, 최소 상승 확률, 워치리스트 여부, 기존 보유 제외 여부를 직접 설정해 그 범위 안에서 신규 자금용 추천 비중안을 다시 계산 |
| **최적 추천** | 현재 포트폴리오의 집중도와 각 시장 체제를 함께 읽고, 지금 기준으로 가장 효율적인 신규 자금 배분안을 자동으로 제안 |
| **Daily Ideal Portfolio** | 한국·미국·일본 레이더를 합쳐 다음 거래일 기준 가장 이상적인 종목 조합과 목표 비중을 매일 생성하고, 이전 추천안의 실제 성과까지 날짜별로 추적 |
| **Daily Market Briefing** | 선택한 국가 대시보드와 추천 카드의 기반이 되는 장 상태, 국가별 체제, 오늘 우선 볼 종목, 이벤트 요약 |
...
| **Market Session Monitor** | `/api/market/sessions` 에서 한국·미국·일본 장의 현재 상태, 마지막 완결 종가 날짜, 다음 거래일 기준점을 함께 반환 |
...
| **티커 자동 해석** | `005930`, `8001`, `KRX:005930`, `TYO:8001`, `BRK.B` 같은 입력을 시장별 표준 Yahoo 티커로 정리해 검색·워치리스트·포트폴리오 전반에 재사용 |
| **워치리스트** | 인라인 추가, 실시간 점수/가격 업데이트, 통화 단위 자동 적용, 기존 raw ticker도 자동 마이그레이션 |
...
| **포트폴리오 이벤트 레이더** | 보유 종목과 국가 비중을 기준으로 다음 3~30일 안에 체크할 실적/거시 일정을 추려 직접 영향 비중까지 표시 |
| **예측 드리프트 모니터** | 종목 상세에서 저장된 다음 거래일 예측 이력을 비교해 상방 확률 변화, 방향 전환, 적중률을 한 번에 점검 |
...
| **워크스페이스 네비게이션** | 좌측 상단 브랜드를 작은 wordmark로 단순화하고, 메뉴를 시장 탐색·운영·리서치 흐름으로 재구성해 불필요한 상단 버튼과 장식 요소를 정리 |
...
| **대시보드 재구성** | 대시보드는 선택 국가의 시장 현황, 히트맵, 상승·하락 상위, 뉴스, 레이더만 바로 읽도록 단순화하고 시스템 상태는 설정 화면으로 분리 |
| **포트폴리오 자산 운영 UX** | 포트폴리오는 총자산 요약, 자산 구성 막대, 총자산 설정, 보유 종목 관리, 리스크·리밸런싱, 추천 순서로 다시 배치해 실제 운영 화면처럼 정리 |
| **포트폴리오 추천 레이아웃 안정화** | 조건 추천/최적 추천 패널은 좁아지면 자연스럽게 세로로 정렬되고, 권장 비중 테이블은 비율을 다시 잡아 깨지기보다 가로 스크롤로 안정적으로 유지 |
...
`frontend/package.json`이 변경된 경우에만 다시 실행하면 됩니다.

### 5. 실행 (매번)

실행 명령은 아래 한 줄만 쓰면 됩니다.

```powershell
& "C:\clone_repo\stock-predict\venv\Scripts\python.exe" "C:\clone_repo\stock-predict\start.py"
```

이 명령은 현재 위치와 무관하게 동작합니다.
`--status`, `--stop`, `--check`가 필요할 때는 같은 명령 뒤에 옵션만 붙이면 됩니다.

예시:

```powershell
& "C:\clone_repo\stock-predict\venv\Scripts\python.exe" "C:\clone_repo\stock-predict\start.py" --stop
```
...
| 절대경로 기준 `start.py` 실행 | - | **O (매번)** |
| `--status` / `--stop` / `--check` 옵션 | - | 필요 시 |
...
| SP-5010 | 티커 해석 실패 | `/api/ticker/resolve` 입력값과 티커 형식 확인 |
| SP-5011 | 일일 마켓 브리핑 조회 실패 | `/api/briefing/daily` 로그 확인 |
| SP-5012 | 시장 세션 요약 조회 실패 | `/api/market/sessions` 로그 확인 |
| SP-5013 | 포트폴리오 이벤트 레이더 조회 실패 | `/api/portfolio/event-radar` 로그 확인 |
| SP-5014 | 예측 드리프트 조회 실패 | `/api/stock/{ticker}/forecast-delta` 로그 확인 |
| SP-5015 | 조건 추천 조회 실패 | `/api/portfolio/recommendations/conditional` 로그 확인 |
| SP-5016 | 최적 추천 조회 실패 | `/api/portfolio/recommendations/optimal` 로그 확인 |
| SP-5017 | 포트폴리오 자산 프로필 저장 실패 | `/api/portfolio/profile` 로그 확인 |
...
| SP-6013 | 잘못된 총자산 프로필 입력값 | 총자산, 예수금, 월 추가 자금을 0 이상의 숫자로 입력 |
