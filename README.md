...
- 종목 상세 `/api/stock/{ticker}/detail`은 이벤트 날짜를 UTC 기준으로 정규화해 계산하며, uncached 상태에서는 빠른 종목 스냅샷을 먼저 내려 첫 클릭이 통째로 빈 실패 화면으로 무너지지 않게 유지합니다. `quick snapshot -> prefer_full 업그레이드` 흐름을 기본으로 두고, Render memory-safe 운영에서는 detached full refresh를 남기지 않도록 같은 요청 안에서만 bounded full detail upgrade를 시도합니다.
- 개별 종목 페이지 `/stock/[ticker]`는 server-first initial snapshot을 사용합니다. 첫 HTML에 partial stock snapshot이 있으면 그대로 보여주고, hydration 뒤에는 partial일 때만 `prefer_full=true` 후속 요청을 한 번 더 보내 full detail로 자연스럽게 교체합니다.
...
- `GET /api/stock/{ticker}/detail`
  ...
  - `fallback_reason=stock_quick_detail`
    - uncached 종목 상세에서 가격 흐름, 기술 신호, 시장 국면을 기준으로 빠른 스냅샷을 먼저 보여줍니다.
    - Render memory-safe 운영에서는 이 quick snapshot 뒤에 무거운 full refresh를 detached background task로 남기지 않고, stock page의 후속 `prefer_full=true` 요청 안에서만 bounded full refresh를 시도합니다.
